...
export class ConfirmationService {
  ...
  private register(pi: ExtensionAPI): void {
    ...
    pi.registerTool({
      ...
      description: "Pause work and ask the user to confirm or cancel a proposed action through a visible confirmation dialog.",
      ...
      execute: async (_toolCallId, params, signal, _onUpdate, context) => {
        ...
        const confirmed = await context.ui.confirm(params.title, params.message);
        signal?.throwIfAborted();
        return {
          content: [{
            type: "text",
            text: confirmed
              ...
              : "The user cancelled the proposed action. Do not perform it.",
          }],
          details: { confirmed },
        };
      },
