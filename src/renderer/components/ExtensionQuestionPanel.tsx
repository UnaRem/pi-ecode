...
export function ExtensionQuestionPanel({ request, onRespond }: ExtensionQuestionPanelProps) {
  ...
  return (
    <section className="extension-question" role="dialog" aria-label="Extension question" onKeyDown={onKeyDown}>
      <header className="extension-question-header">
        <div>
          <span>PI NEEDS YOUR INPUT</span>
          {request.questionCount && request.questionIndex !== undefined && (
            <small>{request.questionIndex + 1}/{request.questionCount}</small>
          )}
        </div>
        <button onClick={() => respond(null)} aria-label="Cancel question"><X size={14} /></button>
      </header>
      ...
      {request.method === "confirm" ? (
        <div className="extension-question-actions">
          ...
          <button className="primary" onClick={() => respond(true)}>Yes</button>
        </div>
        ...
        <form onSubmit={submitText}>
          <textarea
            ...
            placeholder={request.method === "multi-select" ? "Or type a custom answer…" : request.placeholder}
            onChange={(event) => setText(event.target.value)}
            aria-label="Question response"
          />
          <div className="extension-question-actions">
            <button type="button" onClick={() => respond(null)}>Cancel</button>
